...
// LOGICA PARA DECIDIR QUÉ FORMULARIO MOSTRAR
    ...
    if(isset($_GET["form"])){
        switch($_GET["form"]){
            ...
            case "nuevo":
                $action = '?pag=list';
                $target = '';
                $title = $backbtn.'<h1 class="display-5 text-light text-center mb-4">Retorno de cliente</h1>';
                $submit = '<input type="submit" name="nuevo" class="w-100 btn btn-success col-5 mx-auto" value="Guardar Nuevo">';
                break;
            case "edit":
                $action = '?pag=list';
                $target = '';
    ...
    $before = '<form action="'.$action.'" '.$target.' method="POST" class="form-control p-4 bg-dark" enctype="multipart/form-data">'.$title;
